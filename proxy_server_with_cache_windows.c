...
int connectRemoteServe(char* host_addr, int port_num){
    ...
    server_addr.sin_family = htons(port_num);
...
int handle_request(int clientSocketId, ParsedRequest *request, char* tempReq){
    ...
    if(ParsedHeader_Request(request, "Host") == NULL){
        if(ParsedHeader_Request(request, "Host", request->Host)<0){
            printf("Set Host Header ket is not working");
        }
    }
    ...
    while (bytes_send > 0){
        ...
        if (bytes_send<0){
            perror("Error in sending data tp client\n");
            break;
        }
        bzero(buf, MAX_BYTES);
        bytes_send = recv(remoteSocketid,buf,MAX_BYTES-1, 0);
    ...
    if(temp!=NULL){
        ...
        }else
        {
            ...
            if (!strcmp(request->method, "GET"))
            {
                ...
                printf("This code only support GET Method\n");
            }
        }
...
int main(int argc, char* argv[])
{
    ...
    while(1){
        ...
        struct sockaddr_in * client_pt = (struct sockadd_in *)&client_addr;
        struct in_addr ipaddr = client_pt->sin_addr;
        char ipstr[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &ipaddr, ipstr, INET_ADDRSTRLEN);
        printf("Client is connected with port number %d and ipaddress is %s\n", ntohs(client_addr.sin_port), ipstr);
